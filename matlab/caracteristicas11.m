...
DIR_GUARDAR = '../caracteristicas/multiple/';
...
% PACIENTES = 1;
% TSTART      = [33 38 43 48 53];
% TSTOP       = [43 48 53 58 63];
% TRIALS = 1:3;
% clear PARES;
...
potencias_absolutas_bandas = zeros(N_PACIENTES*N_TRIALS_M, N_BANDAS*N_ELECTRODOS);
potencias_absolutas_total  = zeros(N_PACIENTES*N_TRIALS_M, N_ELECTRODOS);
potencias_relativas_bandas = zeros(N_PACIENTES*N_TRIALS_M, N_BANDAS*N_ELECTRODOS);
potencias_asimetria_bandas = zeros(N_PACIENTES*N_TRIALS_M, N_DIF_PARES*N_BANDAS);
potencias_asimetria_total  = zeros(N_PACIENTES*N_TRIALS_M, N_DIF_PARES);
...
for p = 1:N_PACIENTES
    ...
    % [p_theta, p_salpha, p_alpha, p_beta, p_gamma] = calcularPotencias1(N_ELECTRODOS, N_TRIALS_M, PARES, s_eeg_theta, s_eeg_salpha, s_eeg_alpha, s_eeg_beta, s_eeg_gamma);
    [p_theta, p_theta_r, p_salpha, p_salpha_r, p_alpha, p_alpha_r, p_beta, p_beta_r, p_gamma, p_gamma_r, p_total] = ...
    calcularPotencias1(N_ELECTRODOS, N_TRIALS_M, PARES, s_eeg_theta, s_eeg_salpha, s_eeg_alpha, s_eeg_beta, s_eeg_gamma);
    ...
    potencias_absolutas_bandas(1+(p-1)*N_TRIALS_M:p*N_TRIALS_M,theta_i)  = reshape(p_theta(1:N_ELECTRODOS*N_TRIALS_M),  N_ELECTRODOS, N_TRIALS_M)';
    potencias_absolutas_bandas(1+(p-1)*N_TRIALS_M:p*N_TRIALS_M,salpha_i) = reshape(p_salpha(1:N_ELECTRODOS*N_TRIALS_M), N_ELECTRODOS, N_TRIALS_M)';
    potencias_absolutas_bandas(1+(p-1)*N_TRIALS_M:p*N_TRIALS_M,alpha_i)  = reshape(p_alpha(1:N_ELECTRODOS*N_TRIALS_M),  N_ELECTRODOS, N_TRIALS_M)';
    potencias_absolutas_bandas(1+(p-1)*N_TRIALS_M:p*N_TRIALS_M,beta_i)   = reshape(p_beta(1:N_ELECTRODOS*N_TRIALS_M),   N_ELECTRODOS, N_TRIALS_M)';
    potencias_absolutas_bandas(1+(p-1)*N_TRIALS_M:p*N_TRIALS_M,gamma_i)  = reshape(p_gamma(1:N_ELECTRODOS*N_TRIALS_M),  N_ELECTRODOS, N_TRIALS_M)';

    % 2) Potencia total absoluta en (por electrodo)
    potencias_absolutas_total(1+(p-1)*N_TRIALS_M:p*N_TRIALS_M,:) = reshape(p_total(1:N_ELECTRODOS*N_TRIALS_M), N_ELECTRODOS, N_TRIALS_M)';

    % 3) Potencias relativas en cada banda (por electrodo)
    potencias_relativas_bandas(1+(p-1)*N_TRIALS_M:p*N_TRIALS_M,theta_i)  = reshape(p_theta_r,  N_ELECTRODOS, N_TRIALS_M)';
    potencias_relativas_bandas(1+(p-1)*N_TRIALS_M:p*N_TRIALS_M,salpha_i) = reshape(p_salpha_r, N_ELECTRODOS, N_TRIALS_M)';
    potencias_relativas_bandas(1+(p-1)*N_TRIALS_M:p*N_TRIALS_M,alpha_i)  = reshape(p_alpha_r,  N_ELECTRODOS, N_TRIALS_M)';
    potencias_relativas_bandas(1+(p-1)*N_TRIALS_M:p*N_TRIALS_M,beta_i)   = reshape(p_beta_r,   N_ELECTRODOS, N_TRIALS_M)';
    potencias_relativas_bandas(1+(p-1)*N_TRIALS_M:p*N_TRIALS_M,gamma_i)  = reshape(p_gamma_r,  N_ELECTRODOS, N_TRIALS_M)';
    ...
    potencias_asimetria_bandas(1+(p-1)*N_TRIALS_M:p*N_TRIALS_M, theta_i)  = reshape(p_theta(1+N_ELECTRODOS*N_TRIALS_M:end),  N_DIF_PARES, N_TRIALS_M)';
    potencias_asimetria_bandas(1+(p-1)*N_TRIALS_M:p*N_TRIALS_M, salpha_i) = reshape(p_salpha(1+N_ELECTRODOS*N_TRIALS_M:end), N_DIF_PARES, N_TRIALS_M)';
    potencias_asimetria_bandas(1+(p-1)*N_TRIALS_M:p*N_TRIALS_M, alpha_i)  = reshape(p_alpha(1+N_ELECTRODOS*N_TRIALS_M:end),  N_DIF_PARES, N_TRIALS_M)';
    potencias_asimetria_bandas(1+(p-1)*N_TRIALS_M:p*N_TRIALS_M, beta_i)   = reshape(p_beta(1+N_ELECTRODOS*N_TRIALS_M:end),   N_DIF_PARES, N_TRIALS_M)';
    potencias_asimetria_bandas(1+(p-1)*N_TRIALS_M:p*N_TRIALS_M, gamma_i)  = reshape(p_gamma(1+N_ELECTRODOS*N_TRIALS_M:end),  N_DIF_PARES, N_TRIALS_M)';

    % 5) Asímetría potencia total (pares electrodos simétricos)
    potencias_asimetria_total(1+(p-1)*N_TRIALS_M:p*N_TRIALS_M,:) = reshape(p_total(1+N_ELECTRODOS*N_TRIALS_M:end), N_DIF_PARES, N_TRIALS_M)';
...
for p = 1:N_PACIENTES
    paciente = PACIENTES(p);
    c1_i     = (p-1)*N_TRIALS_M + find(etiquetas_valence(1+(p-1)*N_TRIALS_M:p*N_TRIALS_M)<=PUNTUACION_CLASE1); 
    if (PUNTUACION_CLASE1 == PUNTUACION_CLASE2)
        c2_i = (p-1)*N_TRIALS_M + find(etiquetas_valence(1+(p-1)*N_TRIALS_M:p*N_TRIALS_M)>PUNTUACION_CLASE2);
    else 
        c2_i = (p-1)*N_TRIALS_M + find(etiquetas_valence(1+(p-1)*N_TRIALS_M:p*N_TRIALS_M)>=PUNTUACION_CLASE2);
    end
...
for p = 1:N_PACIENTES
    paciente = PACIENTES(p);
    c1_i    = (p-1)*N_TRIALS_M + find(etiquetas_arousal(1+(p-1)*N_TRIALS_M:p*N_TRIALS_M)<=PUNTUACION_CLASE1); 
    if (PUNTUACION_CLASE1 == PUNTUACION_CLASE2)
        c2_i = (p-1)*N_TRIALS_M + find(etiquetas_arousal(1+(p-1)*N_TRIALS_M:p*N_TRIALS_M)>PUNTUACION_CLASE2);
    else 
        c2_i = (p-1)*N_TRIALS_M + find(etiquetas_arousal(1+(p-1)*N_TRIALS_M:p*N_TRIALS_M)>=PUNTUACION_CLASE2);
    end
    ...
    ntrials_paciente_a(p) = length(t_);
